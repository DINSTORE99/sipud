...
export default function App() {
  ...
  useEffect(() => {
    supabase.from("visitors").select("id").limit(1).then(({ error }) => {
      if (error) console.error("Supabase connection:", error);
      else setSupabaseReady(true);
    });
  ...
  const handleSubmit = async (e) => {
    ...
    sendTelegramNotification("process", { url, platform: selectedPlatform });
